...
async function sendEmail(type: string, to: string, name: string) {
  try {
    await fetch("/api/email", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ type, to, name }) });
  } catch {}
}
  ...
  return (
    <div className="min-h-screen flex flex-col px-4 py-8"
      ...
      <div className="flex items-center gap-2.5 mb-6">
        ...
        <span className="font-syne font-bold text-xl text-foreground">UDI<span style={{ color: "#3B82F6" }}>HUB</span></span>
      </div>
      {role === "professional" && step === 2 && (
        <div className="flex items-center gap-2 mb-6">
          <div className="flex items-center gap-1.5">
            <div className="w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-bold"
              style={{ background: "rgba(34,197,94,0.2)", color: "#22c55e" }}><Check size={12} /></div>
      ...
      {step === 1 && (
        <form onSubmit={handleStep1} className="space-y-4 max-w-lg w-full mx-auto">
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-2">
              <button type="button" onClick={() => setRole("client")}
                ...
                <div><p className="font-syne font-bold text-sm text-foreground">Cliente</p><p className="text-[10px] text-muted">Busco servicos</p></div>
              </button>
              <button type="button" onClick={() => setRole("professional")}
                ...
                <div><p className="font-syne font-bold text-sm text-foreground">Profissional</p><p className="text-[10px] text-muted">Recebo clientes</p></div>
              </button>
            </div>
            {role === "professional" && <p className="text-[10px] mt-2" style={{ color: "#22c55e" }}>✓ Comece de graça — ative seu perfil agora mesmo, sem cartão</p>}
          </div>
          <div>
            <label className="block text-xs font-medium text-muted mb-1.5">Nome completo *</label>
            <input type="text" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="Seu nome completo" required className={inputClass} style={inputStyle} />
          </div>
          <div>
            <label className="block text-xs font-medium text-muted mb-1.5">E-mail *</label>
            <input type="email" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} placeholder="seu@email.com" required className={inputClass} style={inputStyle} />
          </div>
          <div>
            <label className="block text-xs font-medium text-muted mb-1.5">WhatsApp {role === "professional" ? "*" : "(opcional)"}</label>
            <input type="tel" value={form.phone} onChange={(e) => setForm({ ...form, phone: formatPhone(e.target.value) })}
              placeholder="(34) 99999-9999" required={role === "professional"} inputMode="numeric" maxLength={16} className={inputClass} style={inputStyle} />
            {role === "professional" && <p className="text-[10px] text-muted mt-1">Este numero recebera os contatos dos clientes</p>}
          </div>
          <div>
            <label className="block text-xs font-medium text-muted mb-1.5">Senha *</label>
            <input type="password" value={form.password} onChange={(e) => setForm({ ...form, password: e.target.value })} placeholder="Minimo 6 caracteres" minLength={6} required className={inputClass} style={inputStyle} />
          </div>
          <button type="submit" disabled={loading || !role}
            ...
            style={{ background: "linear-gradient(135deg, #3B82F6, #1d4ed8)", boxShadow: "0 0 20px rgba(59,130,246,0.3)", opacity: (loading || !role) ? 0.7 : 1 }}>
            {loading ? <><Loader2 size={16} className="animate-spin" /> Processando...</> : role === "professional" ? <> Continuar <ArrowRight size={16} /></> : "Criar conta"}
          </button>
          <p className="text-center text-sm text-muted pt-2">Ja tem conta?{" "}
            <Link href={redirectTo ? `/login?redirect=${encodeURIComponent(redirectTo)}` : "/login"} className="font-semibold" style={{ color: "#3B82F6" }}>Entrar</Link>
          </p>
        </form>
      )}
        ...
        <form onSubmit={handleFinalSubmit} className="space-y-4 max-w-lg w-full mx-auto">
          ...
          <div>
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="flex-1">
                <p className="text-xs text-muted leading-relaxed">Perfis com foto recebem <strong className="text-foreground">3x mais</strong> contatos.</p>
                {!avatarPreview && <p className="text-[10px] mt-1" style={{ color: "#f87171" }}>⚠️ Foto obrigatória para criar perfil</p>}
              </div>
            </div>
          </div>
          {/* Especialidade */}
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input type="text" value={form.instagram} onChange={(e) => setForm({ ...form, instagram: e.target.value.replace("@", "") })}
          ...
          <div>
            ...
            <textarea value={form.bio} onChange={(e) => setForm({ ...form, bio: e.target.value })}
              placeholder="Fale sobre sua experiencia e servicos..." rows={3} maxLength={300}
          ...
          <div className="p-4 rounded-2xl" style={{ background: "rgba(34,197,94,0.06)", border: "1px solid rgba(34,197,94,0.25)" }}>
            ...
            <p className="text-xs text-muted leading-relaxed">Ative agora sem pagar nada e receba até 5 clientes únicos por mês. Quer clientes ilimitados? Faça upgrade quando quiser, direto no seu painel.</p>
          </div>
          <button type="submit" disabled={loading}
            ...
            {loading ? <><Loader2 size={16} className="animate-spin" /> Criando perfil...</> : "Criar perfil gratuito"}
          </button>
        </form>
      )}
    </div>
...
export default function CadastroPage() {
  return (
    <Suspense fallback={<div className="min-h-screen flex items-center justify-center bg-background"><Loader2 size={24} className="animate-spin text-muted" /></div>}>
      <CadastroForm />
    </Suspense>
  );
}
