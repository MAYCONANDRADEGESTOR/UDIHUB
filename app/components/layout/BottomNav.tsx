...
export default function BottomNav() {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-50"
      style={{ background: "rgba(9,9,11,0.97)", borderTop: "1px solid #1F1F23" }}>
      ...
      <div className="flex items-center justify-around px-2 pt-2 pb-3">
        {navItems.map(({ href, icon: Icon, label, cta }) => {
          ...
          if (cta) return (
            <Link key={label} href={href || "/seja-profissional"}
          ...
          // Painel — só renderiza quando href estiver carregado
          if (label === "Painel" && !painelHref) return (
            <div key={label} className="flex flex-col items-center gap-1 min-w-[56px] py-1">
